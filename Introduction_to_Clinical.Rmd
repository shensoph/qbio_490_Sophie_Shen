...
```{r}
write.csv(clinic, "~/490_cluster/analysis_data/GDCdata/TCGA-BRCA/Clinical/Clinical_Supplement.csv", row.names=FALSE)
```

To read the csv back into your environment as a dataframe:

```{r}
clinic <- read.csv("~/490_cluster/analysis_data/GDCdata/TCGA-BRCA/Clinical/Clinical_Supplement.csv")
```
...
# 4. How old was the 1st patient at their last follow up (barcode: TCGA-3C-AAAU)? 45
...
```{r Exercise 4.1}
plot(clinic$birth_days_to, clinic$last_contact_days_to)
```
...
```{r Exercise 4.2}
boxplot(clinic$age_at_diagnosis ~ clinic$tumor_status)
```
...
cleaned_clinic$age_category <- ifelse(cleaned_clinic$age_at_diagnosis <= 35, "Young",
                                      ifelse(cleaned_clinic$age_at_diagnosis >= 50, "Old", 
                                             "Middle"))
```
...
# 4. What age group appears to have the worst rate of survival? Old

# 5. What is the approx. 15-year survival rate for "Old" breast cancer patients? ~25%

# 6. What is the p-value? Is it statistically significant and what does that tell us about the relationship between age and breast cancer survival? the p value is 0.2. The p value is > 0.05, which tells us that there is not suffieciently strong evidence to conclude a correlation between age and breast cancer survival.
